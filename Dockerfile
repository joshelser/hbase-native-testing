...
RUN apt-get update && apt-get install -y curl git wget binutils less libterm-readline-gnu-perl g++ libgflags-dev libgoogle-glog-dev vim build-essential dh-autoreconf pkg-config libssl-dev libboost-all-dev libevent-dev libdouble-conversion-dev cmake libprotobuf-dev libsasl2-dev libkrb5-dev
...
RUN cd /root/build/folly-2017.09.04.00/folly \
      && autoreconf -ivf \
      && ./configure \
      && make -j8 \
      && make install \
